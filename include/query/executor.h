...
class Executor {
public:
    Executor(const std::string &data_root, std::shared_ptr<LruCache> cache);

    QueryResult execute_sql(const std::string &sql, std::string &current_database);
    QueryResult execute_batch_insert(const std::vector<std::string> &sql_batch, std::string &current_database);

private:
    QueryResult execute(const QueryPlan &plan);
    std::shared_ptr<Table> get_table(const std::string &database_name, const std::string &table_name, std::string &error);
    QueryResult execute_create_database(const Query &query);
    QueryResult execute_use_database(const Query &query);
    QueryResult execute_create(const Query &query);
    QueryResult execute_drop_table(const Query &query);
    QueryResult execute_insert(const Query &query);
    QueryResult execute_select(const Query &query);
    QueryResult execute_show_databases();
    QueryResult execute_show_tables(const Query &query);
    QueryResult execute_simple_select(Table &table, const Query &query, const std::string &normalized_sql);
    QueryResult execute_join_select(Table &left, const Query &query, const std::string &normalized_sql);
