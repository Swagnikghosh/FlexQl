...
bool vectorized_equals(std::string_view left_view, std::string_view right_view) {
    const std::size_t length = left_view.size();
    if (length != right_view.size()) {
        return false;
    }
    ...
    if (length >= kSimdWidth) {
        ...
        return std::memcmp(left_ptr + cursor, right_ptr + cursor, length - cursor) == 0;
    }
#endif

    return std::memcmp(left_view.data(), right_view.data(), length) == 0;
}
