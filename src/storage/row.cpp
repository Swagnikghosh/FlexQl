...
template <typename T>
void append_pod(std::string &buffer, const T &pod_value) {
    buffer.append(reinterpret_cast<const char *>(&pod_value), sizeof(T));
}

template <typename T>
bool read_pod(std::string_view source, std::size_t &cursor, T &pod_value) {
    if (cursor + sizeof(T) > source.size()) {
        return false;
    }

    std::memcpy(&pod_value, source.data() + cursor, sizeof(T));
    cursor += sizeof(T);
    return true;
}
...
std::string serialize_row(const Row &row) {
    ...
    encoded_row.reserve(sizeof(std::uint32_t) + payload_bytes);

    append_pod(encoded_row, static_cast<std::uint32_t>(payload_bytes));
    append_pod(encoded_row, static_cast<std::int64_t>(row.expiration));
    append_pod(encoded_row, static_cast<std::uint32_t>(row.values.size()));
    for (const auto &field_value : row.values) {
        append_pod(encoded_row, static_cast<std::uint32_t>(field_value.size()));
        encoded_row.append(field_value.data(), field_value.size());
    }
    return encoded_row;
}
