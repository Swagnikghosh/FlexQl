...
void Schema::set_columns(std::vector<ColumnDef> column_defs) {
    columns_ = std::move(column_defs);
}
...
std::string Schema::primary_key_name() const {
    const int key_index = primary_key_index();
    if (key_index < 0) {
        return "";
    }

    return columns_[key_index].name;
}
