...
bool send_all(int socket_fd, const char *buffer, std::size_t byte_count, std::string &failure_reason) {
...
bool recv_all(int socket_fd, char *buffer, std::size_t byte_count, std::string &failure_reason) {
...
bool SocketClient::connect_to(
    ...
    struct addrinfo query_hints {};
    query_hints.ai_family = AF_UNSPEC;
    query_hints.ai_socktype = SOCK_STREAM;

    struct addrinfo *address_list = nullptr;
    const std::string service_name = std::to_string(port_number);
    if (::getaddrinfo(host_name.c_str(), service_name.c_str(), &query_hints, &address_list) != 0) {
