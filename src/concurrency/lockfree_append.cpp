...
bool LockFreeAppendFile::open(const std::string &file_path, std::string &failure_reason) {
    if (fd_ >= 0) {
        return true;
    }
    ...
    if (opened_fd < 0) {
        failure_reason = "failed to open append file";
        return false;
    }

    struct stat file_stats {};
    if (::fstat(opened_fd, &file_stats) != 0) {
        failure_reason = "failed to stat append file";
        ::close(opened_fd);
        return false;
    }
...
std::uint64_t LockFreeAppendFile::reserve(std::size_t requested_bytes) {
    const std::uint64_t reserved_offset = append_offset_.fetch_add(
        static_cast<std::uint64_t>(requested_bytes),
        std::memory_order_acq_rel);
    maybe_extend(reserved_offset + static_cast<std::uint64_t>(requested_bytes));
...
bool LockFreeAppendFile::write_at(
    ...
    if (fd_ < 0) {
        failure_reason = "append file is not open";
        return false;
    }
    ...
    while (bytes_written < buffer.size()) {
        ...
        if (write_result < 0) {
            ...
            failure_reason = "failed to append row";
            return false;
        }
