...
void erase_dependency_links(
    ...
    for (auto map_it = dependency_links.begin(); map_it != dependency_links.end();) {
        map_it->second.erase(entry_key);
        if (map_it->second.empty()) {
            map_it = dependency_links.erase(map_it);
        } else {
            ++map_it;
        }
    }
...
bool LruCache::get(const std::string &lookup_key, QueryResult &cached_value) {
    ...
    cached_value = entry_it->second.it->second;
    return true;
}
...
void LruCache::put(
    ...
    if (existing != map_.end()) {
        existing->second.it->second = result;
    ...
    probation_.push_front({entry_key, result});
