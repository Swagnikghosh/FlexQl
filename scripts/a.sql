-- FlexQL validation script generated by scripts/generate_validation_sql.sh
-- Run from the FlexQL client REPL with:
-- SOURCE scripts/check_all_queries_smoke.sql
...
CREATE TABLE smoke_main(ID INT PRIMARY KEY, C001 VARCHAR(255), C002 VARCHAR(255), C003 VARCHAR(255), C004 VARCHAR(255), C005 VARCHAR(255), C006 VARCHAR(255), C007 VARCHAR(255));
CREATE TABLE smoke_lookup(ID INT PRIMARY KEY, TAG VARCHAR(255), CREATED_AT DATETIME);
CREATE TABLE smoke_expiry(ID INT PRIMARY KEY, NAME VARCHAR(255));
SHOW TABLES;
...
INSERT INTO smoke_expiry VALUES (1, 'expired-row') EXPIRES AT '2000-01-01 00:00:00';
SELECT * FROM smoke_main WHERE ID = 1;
SELECT * FROM smoke_main WHERE ID = 25;
SELECT ID FROM smoke_main WHERE C001 = 'r00025_c001';
SELECT smoke_main.C001, smoke_lookup.TAG FROM smoke_main INNER JOIN smoke_lookup ON smoke_main.ID = smoke_lookup.ID WHERE smoke_lookup.ID = 25;
